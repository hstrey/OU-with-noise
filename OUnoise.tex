...
\node[main node,minimum size=1.5cm] (1) {$x_{1}$};
\node[main node,minimum size=1.5cm] (2) [right of=1] {$x_{2}$};
\node[main node,minimum size=1.5cm] (4) [right of=2] {$x_{n-1}$};
\node[main node,minimum size=1.5cm] (5) [right of=4] {$x_{n}$};
\node[main node,minimum size=1.5cm] (6) [right of=5] {$x_{n+1}$};
\node[main node,minimum size=1.5cm] (8) [right of=6] {$x_{N-1}$};
\node[main node,minimum size=1.5cm] (9) [right of=8] {$x_{N}$};
\node at ($(2)!.5!(4)$) {\ldots};
\node at ($(6)!.5!(8)$) {\ldots};

\node[main node,minimum size=1.5cm] (10) [below of=1] {$y_{1}$};
\node[main node,minimum size=1.5cm] (11) [below of=2] {$y_{2}$};
\node[main node,minimum size=1.5cm] (12) [below of=4] {$y_{n-1}$};
\node[main node,minimum size=1.5cm] (13) [below of=5] {$y_{n}$};
\node[main node,minimum size=1.5cm] (14) [below of=6] {$y_{n-1}$};
\node[main node,minimum size=1.5cm] (15) [below of=8] {$y_{N-1}$};
\node[main node,minimum size=1.5cm] (16) [below of=9] {$y_{N}$};
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
here we took advantage of the properties of the Markov chain $\{x_{i}\}$.  If we know a specific $x_{n}$ then $x_{n-1}$ and $x_{n+1}$ are independent because the path is blocked.  As a consequence of that, the $y_{i}$ with $i\leq n$ are independent of $y_{i}$ with $i>n$.
